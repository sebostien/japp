...
impl ExprParser {
    ...
    pub fn parse(&mut self, mut source: Vec<String>) -> Result<Expr, String> {
        self.pos = 0;
    ...
    fn parse_expr(&mut self, tokens: &mut Vec<String>, precedence: usize) -> Result<Expr, String> {
        let mut lhs = self.primary(tokens)?;

        while self.pos < tokens.len() {
            let op = tokens[self.pos].clone();
            println!("E: {op:?}");
    ...
    fn primary(&mut self, tokens: &mut Vec<String>) -> Result<Expr, String> {
        ...
        if token == "(" {
            ...
            match Lit::from(&token) {
                Lit::Ident(i) => {
                    if self.pos < tokens.len() && tokens[self.pos] == "(" {
    ...
    fn parse_f_call(&mut self, tokens: &mut Vec<String>, ident: String) -> Result<Expr, String> {
        let mut args = vec![];
        self.pos += 1; // Consume '('
